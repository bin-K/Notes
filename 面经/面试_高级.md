...
//已知有字符串foo='get-element-by-id',写一个function将其转化成驼峰表示法"getElementById
        ...
        console.log(change(str2));
```

## （4）数组反转

```javascript
 // 原理：最后一个跟第一个数进行交换，第二个数跟倒数第二个数进行交换以此类推，总交换次数是数组长度的一半
...
<body>
  ...
  <script>
    /*
    函数的节流:一次函数执行后,只有大于设定的执行周期后才会执行第二次
    - 有个需要频繁触发的函数,处于性能优化的考虑,在规定时间内,只让函数第一次触发生效,后面的触发不生效
    ...
    */
    ...
    document.onscroll = throttle(function () { console.log('scroll事件触发了' + Date.now()); }, 200)

    /*
    函数防抖:一个频繁触发的函数,在规定时间内,只让最后一次生效,前面触发不生效
    */
    ...
    document.querySelector('button').onclick = debounce(function() {console.log('点击事件被触发了'+Date.now());},1000)
  </script>
</body>
...
// - cur 表示当前正在处理的数组元素;
// - index表示当前正在处理的数组元素的索引，若提供 init值，则索引为0，否则索引为1;
// - init表示初始值。
var arr = [1，2，3，4]
//求和
var sum = arr.reduce((pre,cur)=> { return pre + cur })
//求积
var mul = arr.reduce((pre,cur)=> { return pre * cur })
//计算每个元素出现的额个数
...
var myCounter = MyCounter + 1 //NaN,与引入的myCounter，无关，此时的myCounter是一个变量，undefined+1 =NaN
```
...
//都能改变this的指向，传入参数的方式不一样，call传入参数只能一个一个来传，而apply传入参数传入的是一个数组
...
function isInstanceOf(instance, klass) {
  ...
  let proto  = Object.getPrototype(instance)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8823cf14d07040379eb890c09255c35d~tplv-k3u1fbpfcp-watermark.image)
